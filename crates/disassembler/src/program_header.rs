...
impl ProgramHeader {
    /// Parse the program headers, recording invalid `p_type` values in
    /// `errors`. A header with an unknown type is kept with a `PT_NULL`
    /// placeholder so header indices stay aligned with the file.
...
mod tests {
    ...
    fn test_program_headers() {
        ...
        let parsed = Program::from_bytes(&original_bytes).unwrap();
