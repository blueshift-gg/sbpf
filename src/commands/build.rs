...
pub struct BuildArgs {
    ...
    #[arg(short = 'i', long, help = "Path to the input assembly file")]
...
fn compile_assembly(src: &str, deploy: &str, debug: bool, arch: SbpfArch) -> Result<()> {
    ...
    // write bytecode to <filename>.so or <deploypath>/<filename>.so
    let output_path = Path::new(deploy).join(file_name).with_extension("so");
...
fn compile_result(path: &str, deploy: &str, debug: bool, arch: SbpfArch) -> Result<()> {
    println!(
        "⚡️ Building \"{}\"{}",
        path,
        if debug { " (debug)" } else { "" }
...
pub fn build(args: BuildArgs) -> Result<()> {
    let is_local_build = args.input.is_some();
    let deploy = args
        .deploy_dir
        .as_deref()
        .unwrap_or(if is_local_build { "." } else { "deploy" });

    // Create necessary directories
    create_dir_all(deploy)?;

    if is_local_build {
        compile_result(&args.input.unwrap(), deploy, args.debug, args.arch.into())?;
    } else {
        ...
        for entry in entries {
                ...
                && let Some(subdir) = path.file_name().and_then(|name| name.to_str())
            {
                ...
                if Path::new(&asm_file).exists() {
                    compile_result(&asm_file, deploy, args.debug, args.arch.into())?;
                }
            }
        }
    }
