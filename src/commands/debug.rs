...
pub fn debug(args: &DebugArgs) -> Result<()> {
    ...
    let session = match (&args.asm, &args.elf) {
        (Some(asm_path), None) => load_session_from_asm(asm_path.as_str(), input_bytes, config)?,
        (None, Some(elf_path)) => load_session_from_elf(elf_path.as_str(), input_bytes, config)?,
        _ => {
